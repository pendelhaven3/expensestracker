...
public class SearchExpensesListController extends AbstractController {
	...
	@Parameter private ExpenseSearchCriteria criteria;
	...
	private void createTabs() {
		...
		for (Category1 category1 : categoryService.getAllLevel1Categories()) {
			...
			if (category1.equals(this.category1)) {
				category1TabPane.getSelectionModel().select(category1Tab);
			}
			...
			for (Category2 category2 : categoryService.findAllCategory2ByParent(category1)) {
				...
				if (!StringUtils.isEmpty(category2.getShortDescription())) {
					category2Tab.setText(category2.getShortDescription());
				} else {
				...
				if (category2.equals(this.category2)) {
					category2TabPane.getSelectionModel().select(category2Tab);
				}
				...
				if (category3s.isEmpty()) {
					if (!StringUtils.isEmpty(category2.getShortDescription())) {
						...
						hbox.getChildren().add(new Text(category2.getDescription()));
					...
					for (Category3 category3 : categoryService.findAllCategory3ByParent(category2)) {
						...
						if (category3.equals(this.category3)) {
